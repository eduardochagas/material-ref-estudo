...
print('na string, mas vc pode dividir a string a partir de um caractere,')
print('é so colocar como parametro no metodo split().')
...
você pode usar o input() em um codigo sem atribui-lo em uma variável para
...
s[c1:c2] - usado para extrair uma substring de uma string (string maior), extrai
 do primeiro índice até mas não inclusive o último índice.

s[c] - usado para extrair um único caractere de uma string.



O método .split() pode ser usado para criar atribuições multiplas, exemplo:

    (ritmo, vocalista, vocais, baixista) = bandaRock.split() .

 com o arquivo bandaRock acima, estamos usando atribuição multipla para variáveis
 definidas dentro de parênteses para atribuir os valores das linhas divididas
 com split() para as variáveis ritmo, vocalista, vocais e baixista.
 # o número de valores divididos no(s) arquivo(s) tem que ser equivalente ao
 # mesmo número de váriáveis definidas na atribuição multipla.
